...
function WorkingSchedule() {
    ...
    const [dataForWorkSchedule, setDataForWorkSchedule] = useState([]);
    var now = dayjs()
    ...
    useEffect(()=>{
        ...
        setDataForWorkSchedule(dataForWorkSchedule)
        setValue(dataForWorkSchedule)
  ...
  const onSelect = (value) => {
   ...
   const onChangeDate = (date) => {
        const fetchData = dataForWorkSchedule
        if(fetchData){
            ...
            const filteredData = dataForWorkSchedule.filter((item) => dayjs(item.date).isSame(date, 'day'));
            setValue(filteredData)
        }
    console.log('ngày hiện tại', date.format('YYYY-MM-DD') )
    console.log('name', value )
    console.log('name2', dataForWorkSchedule )
   }
    ...
    return ( 
        ...
        <div className="w-screen h-auto lg:flex lg:justify-center">
           ...
           <div className="w-full lg:w-[1000px] px-2 space-y-5">
                <h1 className="text-[28px] lg:text-[32px]  font-medium text-[#ff1e56] text-center my-3 lg:my-6">Work Schedule</h1>
